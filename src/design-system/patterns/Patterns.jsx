...
function ObjectPatternFrame({ category, mainTags, objectDescription, tagCloud, additionalSections = [], reviews, bookingSteps, relatedListings, faqSection, bookingWidget }) {
  return <section className={`ds-pattern ds-object-pattern ds-object-pattern--${category}`} aria-label={`${category} details`}>
    ...
    <div className="ds-object-pattern__layout">
      <div className="ds-object-pattern__flow">{objectDescription}{tagCloud}<RegisteredAdditionalSections sections={additionalSections} />{reviews}{bookingSteps}{faqSection}</div>
...
export const PATTERN_CONTRACTS = {
  ...
  objectDetail: { task: 'Understand a concrete offer, compare its facts and submit a category-aware request.', sequence: ['ObjectMainTags', 'ObjectDescription', 'ObjectTagCloud?', 'RegisteredAdditionalSections?', 'ObjectReviews?', 'BookingSteps?', 'FaqAccordion?', 'BookingWidget?', 'ObjectRelatedListings?'], states: ['ready', 'unavailable'], interaction: 'The booking widget remains available while reading; mobile summary expands into the complete request form.', mobile: 'Four main tags become a 2×2 grid; content becomes one column; booking becomes a fixed bottom disclosure.', accessibility: 'Section headings begin at H2, review disclosure exposes aria-expanded, form controls keep visible labels and the mobile widget is keyboard operable.', variations: ['instructor', 'activity', 'rental', 'transfer', 'stay'] }
};
