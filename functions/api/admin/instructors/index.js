...
export async function onRequestPost({ request, env }) {
  if (!await isAuthenticated(request, env)) return apiError('Unauthorized', 401);
  try { return json({ data: await saveInstructor(env.DB, await request.json(), undefined, { bucket: env.MEDIA }) }, { status: 201, cacheControl: 'no-store' }); } catch (error) { return apiError(error.message, 400); }
}
