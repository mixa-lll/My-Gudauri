...
function sharedProps(item, props = {}) {
  ...
  return {
    ...
    image: item.image,
    imageAlt: title,
    placeholderKind: props.placeholderKind,
    title,
...
export function StayCard({ item, basePath = '/stays', ...props }) {
  return <ListingCardFrame {...sharedProps(item, { ...props, placeholderKind: 'stay' })} to={`${basePath}/${item.slug}`} mediaTop={<ListingCardPill>{item.category ?? 'Stay'}</ListingCardPill>} mediaBottom={<Tags items={item.tags} />} footer={<CommerceFooter item={item} />} />;
}
...
export function EditorialCard({ item, basePath = '/articles', ...props }) {
  return <ListingCardFrame {...sharedProps(item, { ...props, placeholderKind: 'editorial' })} to={`${basePath}/${item.slug}`} mediaTop={<ListingCardPill>{item.category ?? 'Article'}</ListingCardPill>} mediaBottom={item.publishedAt ? <ListingCardPill>{item.publishedAt}</ListingCardPill> : null} footer={<ListingCardAction>{item.readingTime ?? item.readTime ?? 'Read article'}</ListingCardAction>} />;
}
